...
const ChatPage = () => {
    ...
    const sendMessageHandler = async (receiverId) => {
        try {
            const res = await axios.post(`https://zubagram-vercel-4tkk.vercel.app/api/v1/message/send/${receiverId}`, { textMessage }, {
                headers: {
    ...
    useEffect(() => {
        const fetchSelectedUser = async () => {
            try {
                if (id) {
                    const res = await axios.get(`https://zubagram-vercel-4tkk.vercel.app/api/v1/user/${id}/profile`, {
                        withCredentials: true
                    });
