...
const CommentDialog = ({ open, setOpen }) => {
  ...
  const sendMessageHandler = async () => {
    try {
      const res = await axios.post(`https://zubagram-vercel-4tkk.vercel.app/api/v1/post/${selectedPost?._id}/comment`, { text }, {
        headers: {
          ...
          <div className='w-1/2 flex flex-col justify-between'>
            ...
            <div className='p-4 bg-gray-900 rounded-b-lg'>
              <div className='flex items-center gap-2'>
                ...
                />
                <Button disabled={!text.trim()} onClick={sendMessageHandler} variant="outline" className='text-white border-gray-600'>Send</Button>
              </div>
            </div>
          </div>
        </div>
